...
(() => {
    const contentGlobal = globalThis;
    ...
    try {
        contentGlobal.__localBrowserBridgeContent?.dispose();
    }
    catch { /* A reloaded extension invalidates the previous runtime object. */ }
    for (const stale of document.querySelectorAll("[data-local-browser-bridge-menu],[data-local-browser-bridge-highlight],[data-codex-browser-bridge-menu]"))
        stale.remove();
    ...
    function dispose() {
        ...
        catch { /* Previous extension runtime may already be invalid. */ }
        document.removeEventListener("contextmenu", onContextMenu, { capture: true });
        document.removeEventListener("pointerdown", onPointerDown, { capture: true });
        document.removeEventListener("keydown", onKeyDown, { capture: true });
        closeMenu();
        if (contentGlobal.__localBrowserBridgeContent === controller)
            delete contentGlobal.__localBrowserBridgeContent;
    }
    function onExtensionMessage(message, _sender, sendResponse) {
        const record = message && typeof message === "object" && !Array.isArray(message) ? message : null;
        if (record?.type === "browserContextReceiver") {
            sendResponse({ name: document.documentElement.getAttribute(receiverAttribute) });
            return;
        }
        if (record?.type !== "injectBrowserContext")
            return;
    ...
    async function initialize() {
        try {
            const initial = await send({ type: "pageInjectionConfig" });
            if (!active || !initial.enabled)
                return;
    ...
    function onContextMenu(event) {
        if (menuHost?.contains(event.target)) {
    ...
    async function captureAndShowContextMenu(x, y, element) {
        closeMenu();
        let screenshot;
        try {
            screenshot = await send({ type: "captureContextScreenshot" });
        }
        catch {
            // A denied or unavailable screenshot must not block page-context capture.
        }
        if (active)
            await showContextMenu(x, y, element.isConnected ? element : document.documentElement, screenshot);
    }
    function onPointerDown(event) {
        if (event.shiftKey && event.button === 2 && !menuHost?.contains(event.target)) {
    ...
    async function showContextMenu(x, y, element, screenshot) {
        ...
        try {
            const config = await send({ type: "pageInjectionConfig" });
            if (!config.enabled) {
                closeMenu();
                return;
            }
            const capturedContext = await captureContext(config, element, screenshot);
            const context = await send({ type: "enrichPageContext", context: capturedContext });
            ...
            actions.append(createButton("Copy", async (button) => {
                ...
                try {
                    await copyText(JSON.stringify(contextWithComment()));
                    setButtonState(button, "Copied", true);
                    setTimeout(closeMenu, 500);
                }
                catch (error) {
                    setButtonState(button, "Copy", false);
                    showError(error);
                }
            }));
            ...
            if (shadow) {
                shadow.replaceChildren(menuStyle());
                shadow.append(createStatus(errorMessage(error)));
            }
        }
    }
    async function captureContext(config, element, screenshot) {
        ...
        await send({ type: "storePageSnapshot", key: contextKey, snapshot });
        return {
    ...
    function positionMenu(host, x, y, element) {
        if (!host)
            return;
        const margin = 12;
        const gap = 10;
        ...
        const clampLeft = (left) => Math.max(margin, Math.min(left, innerWidth - rect.width - margin));
        const clampTop = (top) => Math.max(margin, Math.min(top, innerHeight - rect.height - margin));
        const candidates = [
        ...
        const fits = ({ left, top }) => left >= margin && top >= margin && left + rect.width <= innerWidth - margin && top + rect.height <= innerHeight - margin;
        const overlapArea = ({ left, top }) => Math.max(0, Math.min(left + rect.width, target.right) - Math.max(left, target.left)) *
    ...
    function interceptMenuEvent(event) {
    ...
    function updateElementHighlight() {
        const indicator = highlightHost?.shadowRoot?.querySelector(".indicator");
        if (!indicator || !highlightedElement?.isConnected)
            return;
        ...
        if (badge)
            Object.assign(badge.style, rect.top < 28 ? { top: "0", bottom: "auto" } : { top: "auto", bottom: "100%" });
    }
    ...
    function createStatus(text) { const status = document.createElement("div"); status.className = "status"; status.textContent = text; return status; }
    function setButtonState(button, text, disabled) { button.textContent = text; button.disabled = disabled; }
    function closeMenu() {
        menuHost?.remove();
        menuHost = undefined;
        highlightHost?.remove();
        highlightHost = undefined;
