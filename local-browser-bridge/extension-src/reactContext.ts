...
export function reactContextMetadata(inspection: RecordValue): RecordValue {
  const locations: RecordValue[] = [];
  const indexes = new Map<string, number>();
  const reference = (value: unknown): number | null => {
    if (!record(value) || typeof value.fileName !== "string") return null;
  ...
  const references = (value: unknown): number[] => Array.isArray(value)
    ? [...new Set(value.map(reference).filter((id): id is number => id !== null))] : [];
