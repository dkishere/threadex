...
import { AutoModelSettingsPanel } from "./AutoModelSettingsPanel";
import { GlobalInstructionsSettingsPanel } from "./GlobalInstructionsSettingsPanel";
...
import { ProcessCommandDialog } from "./ProcessCommandDialog";
import { useSubmissionSending } from "./useSubmissionSending";
import { readPendingSubmissions, removePendingSubmission, savePendingSubmission } from "./pendingSubmissions";
...
export function ThreadexShell(ctx) {
  ...
  const { APPROVAL_POLICY_OPTIONS, AUTO_MODEL_VALUE, Activity, ApprovalEvent, ArrowDown, ArrowLeft, ArrowUp, BetweenHorizontalStart, AttachmentList, CONTEXT_FORK_TAG_LABEL, CheckCircle2, CheckSquare2, ChevronDown, ChevronRight, Circle, Clock3, CompletedTurn, Copy, Cpu, Database, Diff, EFFORT_OPTIONS, ExternalLink, FileAnnotationComposerContext, FileText, Folder, FORCE_PLAN_TAG_LABEL, GOAL_MODE_TAG_LABEL, GitFork, InlineLinkComposer, ListChecks, LiveEventList, Loader2, MAX_ATTACHMENTS, MESSAGE_BOTTOM_THRESHOLD, MODEL_OPTIONS, MarkdownContent, MessageTimeline, Pencil, Plus, ProfileSettingsPanel, Quote, ResponseAnnotationList, RotateCcw, Search, Send, ServerPrefixPanels, Settings, ShieldCheck, Square, StatusUpdateIndicator, Target, TerminalSquare, TodoPanel, Trash2, ULTRA_EFFORT_OPTIONS, User, UserPlus, X, _Fragment, _jsx, _jsxs, accountIdentityLabel, accountNeedsLogin, accountResetCredits, annotationLabel, appActions01, appActions02, appActions03, appActions04, appActions05, appActions06, approvalEventToLiveItem, browserBridgeContextAttachmentName, buildCodexReference, buildMessageIndicatorMarks, capitalize, compareSessionRecordsByUpdated, createSystemMessage, displaySessionTitle, earliestExpiringResetCredit, fileChangeItemFromPatchCommand, findSlashTrigger, formatAccountSelectLabel, formatBytes, formatLoadBalanceAccountLabel, formatQuotaPercent, formatQuotaRemaining, formatQuotaReset, formatQuotaStatus, formatResetCreditExpiry, formatTimestamp, formatTimestampShort, formatTokenCount, getSessionExecutionStatus, getWorkspaceTabSummary, groupSessionsByBaseDir, groupTranscriptByStepMarkers, hasVisibleTodoPlan, highlightSessionSearchText, isTodoPlanAwaitingClarification, latestTurnIssueTracker, messageIndicatorMarkTop, messageTimingLabel, modelOptionLabel, movePendingModelPreferences, moveStoredComposerDraft, normalizeComposerDraft, normalizeSessionModelPreferences, normalizeStoredApprovalPolicy, parseBrowserBridgeContext, parseResponseAnnotations, patchStoredComposerDraft, pendingPromptForSubscription, promptDisplayMetadata, queuedPromptSessionKey, quotaWindowsByDuration, readOptionalNavigationTarget, readPendingModelPreferences, readStoredComposerDraft, readStoredModelSelector, readStoredSession, removePendingModelPreferencesIfMatches, sessionExecutionStatusLabel, shortId, shouldRenderMessageTimeline, summarizeTitle, supportsUltraEffort, useCallback, useEventStore, useMemo, useRef, useSessionEffects, useState, writeStoredSession, writePendingModelPreferences, appendSteerSegment, appendTextSegment, applyLiveItemToMessage, approvalDecisionLabel, approvalRecordToLiveItem, cleanLoginUrlValue, composerLinkToken, describeCodexEvent, describeStreamItem, developerInstructionRecordFromPayload, developerInstructionsIndicateForcePlan, elementForSelectionNode, eventStore, finalizeTerminalAssistantMessage, findAssistantMessageId, findUserMessageForTurn, formatComposerLinkMarkdown, formatResponseAnnotationsPrompt, getCodexEventName, isAccountLoginRequiredMessage, isApprovalLiveItem, isCodexTurnCompletedEvent, isInactiveSteerResponse, isLikelyBackendDisconnect, isNoRolloutFoundMessage, itemEventRank, liveItemKey, mergeDeveloperInstructionRecords, moveQueuedPromptInList, moveQueuedPromptToTarget, navigationUrl, nextPastedTextFileName, parseCodexReference, parsePastedHttpUrl, pendingAssistantMessages, promoteQueuedPromptToSteer, readApiError, readAttachment, readEventStream, readNavigationTarget, readRecord, readStringField, reorderPendingTurnMessages, replaceComposerLinkTokens, resetOutcomeLabel, resizeEditor, sessionTurnsToMessages, setsEqual, shouldCompactPastedText, sleep, slugify, toSessionPageState, upsertPendingApprovalItem } = ctx;
    function renderPromptTags(prompt, className = "") {
        ...
        const loopMode = prompt?.executionMode === "loop";
        if (!prompt?.forcePlan && !contextFork && !goalMode && !loopMode) {
            return null;
        }
        return (_jsxs("div", { className: className ? `prompt-tags ${className}` : "prompt-tags", children: [
                ...
                loopMode && _jsx("span", { className: "prompt-tag", "data-tone": "loop", children: "Loop mode" })
            ] }));
    }
    function renderPromptModelBadge(prompt) {
        const model = typeof prompt?.model === "string" ? prompt.model.trim() : "";
        const pending = prompt?.turnStatus === "todo";
        const title = model
            ? `Model used: ${model}${prompt.reasoningEffort ? ` · reasoning effort: ${prompt.reasoningEffort}` : ""}`
            : pending ? "Model will be recorded when this turn starts" : "Model was not recorded for this turn";
        return (_jsxs("span", { className: "prompt-turn-model prompt-turn-toolbar-model", "data-model-recorded": model ? "true" : "false", title, children: [
            model && (prompt.autoModelProvider === "typesafe"
                ? _jsx("span", { className: "prompt-turn-auto-prefix", "data-provider": "typesafe", "data-low-confidence": prompt.autoModelConfidence !== undefined && prompt.autoModelConfidence < 0.3, children: "J " })
                : prompt.autoModel ? _jsx("span", { className: "prompt-turn-auto-prefix", children: "A " }) : null),
            model ? modelOptionLabel(model) : pending ? "Model pending" : "Model not recorded",
            model && prompt.reasoningEffort ? ` · ${shortReasoningEffort(prompt.reasoningEffort)}` : ""
        ] }));
    }
    ...
    const [messages, setSessionMessages] = useState(() => {
        const restored = restoredSession?.messages.length ? restoredSession.messages : [createSystemMessage()];
        return initialNavigationTarget?.view === "workspace-chat" ? recentWorkspaceManagerMessages(restored) : restored;
    });
    ...
    const managerModeRef = useRef(false);
    const managerSessionRef = useRef(null);
    const [managerNavigationRevision, setManagerNavigationRevision] = useState(0);
    const managerEntryRequest = useRef(new URLSearchParams(window.location.search).get("view") === "workspace-chat");
    const setMessages = useCallback((next) => setSessionMessages(current => {
        const updated = typeof next === "function" ? next(current) : next;
        return managerModeRef.current || managerEntryRequest.current ? recentWorkspaceManagerMessages(updated) : updated;
    }), []);
    function sessionSnapshotMessages(turns, ...args) {
        return sessionTurnsToMessages(managerModeRef.current || managerEntryRequest.current ? recentWorkspaceManagerTurns(turns) : turns, ...args);
    }
    ...
    useReactEffect(() => {
        const navigate = () => {
            ...
            setManagerNavigationRevision(value => value + 1);
        };
        window.addEventListener("popstate", navigate);
        return () => window.removeEventListener("popstate", navigate);
    }, []);
    useReactEffect(() => { setSessionTab((current) => ({ sessionId, value: managerMode ? "dashboard" : current.value === "sessions" ? "sessions" : "turns" })); }, [sessionId, managerMode]);
    useReactEffect(() => {
        if (!compactLayout) return;
        setSessionTab((current) => ["sessions", "dashboard"].includes(current.value) ? current : { ...current, value: "sessions" });
    }, [compactLayout]);
    ...
    const [processCommandPrompt, setProcessCommandPrompt] = useState(null);
    const [processMonitorTab, setProcessMonitorTab] = useState("running");
    const registeredCommandIds = new Set(centralState.processMonitors.filter((monitor) => monitor.status === "available").map((monitor) => monitor.id));
    const visibleProcessMonitors = centralState.processMonitors.filter((monitor) => processMonitorTab === "available"
        ? monitor.status === "available"
        : monitor.status !== "available" && !(monitor.status === "exited" && monitor.sourceCommandId && registeredCommandIds.has(monitor.sourceCommandId)));
    ...
    const managerView = useWorkspaceManager(activeWorkspace?.id);
    ...
    const [globalLoopSaving, setGlobalLoopSaving] = useState(false);
    const globalLoopSavingRef = useRef(false);
    useReactEffect(() => {
        let disposed = false;
        const load = async () => {
            try {
                const response = await fetch("/api/loop-mode");
                if (!response.ok) throw new Error(`HTTP ${response.status}`);
                const data = await response.json();
                if (!disposed && !globalLoopSavingRef.current) setGlobalLoopEnabled(data.enabled === true);
            } catch (error) { if (!disposed) console.warn("Could not load Loop mode", error); }
        };
        void load();
        const timer = window.setInterval(() => void load(), 10000);
        return () => { disposed = true; window.clearInterval(timer); };
    }, []);
    async function toggleGlobalLoopMode() {
        if (globalLoopSavingRef.current) return;
        globalLoopSavingRef.current = true;
        setGlobalLoopSaving(true);
        try {
            const response = await fetch("/api/loop-mode", {
                method: "PUT", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ enabled: !globalLoopEnabled })
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            const data = await response.json();
            setGlobalLoopEnabled(data.enabled === true);
        } catch (error) { showToast(`Could not change Loop mode: ${String(error)}`); }
        finally { globalLoopSavingRef.current = false; setGlobalLoopSaving(false); }
    }
    ...
    const { grilledTurns, markGrilled, pendingGrillTurns, pendingGrillSessions } = useGrilledTurns();
    ...
    function setQueuedPrompts(update) {
        ...
        const currentBySession = backgroundQueuesRef.current;
        const current = currentBySession[queueKey] ?? [];
        const next = typeof update === "function" ? update(current) : update;
        if (next === current) return;
        // A dispatched prompt has its own backup until the runner acknowledges it.
        // Only explicit edits/removals of a still-local queue change that backup.
        try {
            const backups = readPendingSubmissions();
            for (const prompt of next) {
                const entry = backups.find(entry => entry.id === prompt.id);
                if (entry && !entry.settings.queued) continue;
                savePendingSubmission({
                    id: prompt.id, turnId: prompt.id, sessionId: sessionIdRef.current,
                    workspaceId: activeWorkspaceIdRef.current, kind: "prompt",
                    createdAt: entry?.createdAt ?? new Date().toISOString(),
                    message: prompt.content, attachments: prompt.attachments,
                    settings: { ...prompt.requestSettings, queued: true, skills: prompt.skills,
                        executionMode: prompt.executionMode, contextFork: prompt.contextFork, forcePlan: prompt.forcePlan }
                });
            }
            for (const entry of backups) {
                if (entry.sessionId !== sessionIdRef.current || !entry.settings.queued) continue;
                const prompt = next.find(prompt => prompt.id === entry.id);
                if (!prompt) removePendingSubmission(entry.id);
            }
        } catch {
            setStatus("Queue edit not saved: unable to save locally.");
            return;
        }
        const nextBySession = { ...currentBySession };
        if (next.length) nextBySession[queueKey] = next;
        else delete nextBySession[queueKey];
        backgroundQueuesRef.current = nextBySession;
        queuedPromptsRef.current = next;
        setQueuedPromptsBySession(nextBySession);
    }
    ...
    const { queued: backendQueuedPrompts, transcript } = useMemo(() => partitionPendingQueueMessages(
    ...
    const activeSessionTab = managerMode ? compactLayout && sessionTab.value === "sessions" ? "sessions" : "dashboard"
        : compactLayout && sessionTab.value === "sessions" ? "sessions" : !["sessions", "dashboard"].includes(sessionTab.value) && sessionTab.sessionId === sessionId && (sessionTab.value !== "plan" || hasPlanTab) ? sessionTab.value : "turns";
    ...
    const visibleSlashSuggestions = useMemo(() => {
        ...
        const modes = [
            ...
            { kind: "mode", mode: "loop", name: "loop", description: "Grill and start work automatically for the next turn" }
        ];
        ...
        try {
            if (open) {
                ...
                const previous = managerReturnSessions.current.get(workspaceId)
                    ?? flatSessions.find(record => record.id !== managerView.state?.manager?.sessionId)?.id;
    ...
    async function clearWorkspaceManager(manager) {
        ...
        setManagerOpening(true);
        try {
            const result = await apiJson("/api/workspace-manager/reset", { method: "POST", body: {
                workspaceId: manager.workspaceId, expectedSessionId: manager.sessionId
            } });
            if (activeWorkspaceIdRef.current !== manager.workspaceId) return;
            await switchToSessionById(result.manager.sessionId, { workspaceManagerView: true, history: "replace" });
            if (sessionIdRef.current !== result.manager.sessionId) throw new Error("New manager was created, but navigation did not complete.");
            managerView.refresh();
            scheduleLoadSessions(0);
            showToast("Fresh manager ready. Previous session archived.");
        } finally {
    ...
    useReactEffect(() => {
        if (!isBootstrapped || !activeWorkspace?.id || !managerEntryRequest.current) return;
        const target = readOptionalNavigationTarget();
    ...
    function applySelectedSessionSnapshot(payload) { if (managerModeRef.current && payload.session?.id !== managerSessionRef.current) return; return appActions02.applySelectedSessionSnapshot({ activeTurnIdRef, applyModelPreferencesState, composerDraftSessionIdRef, composerInputEditRevisionRef, displaySessionTitle, eventStore, explicitNewSessionRef, modelPreferencesHydratedRef, modelPreferencesWorkspaceIdRef, moveStoredComposerDraft, persistedModelPreferencesWorkspaceIdRef, queueModelPreferencesSave, readPendingModelPreferences, replaceComposerDraftForSession, sessionIdRef, sessionTurnsToMessages: sessionSnapshotMessages, setActiveSessionId, setActiveTurnId, setMessages, setResumeThreadId, setRunningTurnIds, setSessionAutoModel, setSessionId, setSessionTodo, setThreadId }, payload); }
    ...
    async function restartProcessMonitor(monitor, parameterValues) {
        if (monitor.status === "available" && monitor.parameters?.length && parameterValues === undefined) {
            setHoveredProcessMonitor(null);
            setProcessCommandPrompt(monitor);
            return false;
        }
        const started = await appActions02.restartProcessMonitor({ eventStore, readApiError, setProcessMonitorAction, setStatus, rethrowErrors: parameterValues !== undefined }, monitor, parameterValues);
        if (started && monitor.status === "available") { setHoveredProcessMonitor(null); setProcessMonitorTab("running"); }
        return started;
    }
    ...
    function updateNavigationUrl(target, mode) { return appActions03.updateNavigationUrl({ navigationTargetRef, navigationUrl }, { ...target, view: managerModeRef.current || managerEntryRequest.current ? "workspace-chat" : null }, mode); }
    ...
    async function deleteBackendQueuedPrompt(turnId) {
        if (pendingQueueActionId) return;
        setPendingQueueActionId(turnId);
        try { await appActions05.deletePendingTurn({ isLikelyBackendDisconnect, noteBackendDisconnect, noteBackendRequestSucceeded, refreshSelectedSessionSnapshot, sessionId, setStatus, showToast }, turnId); }
        finally { setPendingQueueActionId(null); }
    }
    async function steerBackendQueuedPrompt(turnId) {
        if (pendingQueueActionId) return;
        setPendingQueueActionId(turnId);
    ...
    function renderBackendQueuedPrompt({ prompt, response }, index, inTurnRail = false) {
    ...
    function renderMessageArticle(message) {
        ...
        const pendingStatusLabel = message.pendingReason === "rate_limit"
            ? "Waiting for account limit reset"
            : message.pendingReason === "auth"
                ? "Waiting for account login"
                : message.pendingReason === "stopped"
                    ? "Stopped"
                    : "Queued";
...
message.turnStatus === "todo" && (_jsx("span", { className: "turn-status", children: pendingStatusLabel })), message.role === "user" && renderPromptTags(message), message.role === "user" && (_jsx(ServerPrefixPanels, { startupSnapshot: startupSnapshot, developerInstructions: message.developerInstructions, showStartup: message.id === firstUserMessageId })), message.attachments && message.attachments.length > 0 && _jsx(AttachmentList, { attachments: message.attachments }), responseAnnotations && !isEditingUserPrompt && _jsx(ResponseAnnotationList, { annotations: responseAnnotations.annotations }), message.role === "assistant" && message.turnStatus === "done" && message.turnId ? (_jsx(CompletedTurn, { message: message, codexSessionId: threadId, sessionId: sessionId, steerMessages: steerMessages, workspaceId: activeWorkspace?.id })) : shouldRenderMessageTimeline(message) ? (_jsx(MessageTimeline, { anchorPrefix: message.id, completed: message.turnStatus === "done", running: message.turnStatus === "running", statusText: message.runnerStarted === false ? "Connecting" : undefined, segments: message.segments ?? [], codexSessionId: threadId, sessionId: sessionId, turnId: message.turnId, workspaceId: activeWorkspace?.id })) : (_jsxs(_Fragment, { children: [isEditingUserPrompt ? (_jsxs("form", { className: "user-prompt-inline-editor", onSubmit: (event) => void submitInlineUserPromptEdit(event, message), children: [_jsx("textarea", { ref: inlinePromptEditorRef, className: "composer-editor", "aria-label": "Edit prompt", value: inlinePromptEditor?.value ?? "", onChange: (event) => {
    ...
    function renderTurnDetailsModal() {
        ...
        const cachedInputTokenCount = Number.isFinite(promptDetails.cachedInputTokens) ? formatTokenCount(promptDetails.cachedInputTokens) : "Not recorded";
                ...
                >
                    ...
                    <div className="prompt-details-content">
                        <section className="prompt-details-summary" aria-label="Run details">
                            ...
                            <div className="prompt-details-stat"><span>Cached input</span><strong>{cachedInputTokenCount}</strong></div>
    ...
    const shell = (_jsx(FileAnnotationComposerContext.Provider, { value: fileAnnotationContextValue, children: _jsxs("main", { className: "shell", "data-session-tab": activeSessionTab, children: [approvalModal, promptDetailsModal, _jsxs("header", { className: "workspace-header", "aria-label": "Workspaces", children: [_jsxs("div", { className: "workspace-tabs", role: "tablist", "aria-label": "Workspaces", children: [workspaceList.length === 0 && (_jsxs("span", { className: "workspace-tab", "data-active": "true", children: [_jsx(Folder, { "aria-hidden": "true" }), _jsx("span", { children: "Default" })] })), workspaceList.map((workspace) => {
                                    ...
                                    return (_jsxs("div", { className: "workspace-tab-wrap", onMouseEnter: (event) => {
                                ...
                                })] }), _jsx("button", { className: "ghost-icon workspace-add", type: "button", onClick: () => void createWorkspace(), title: "Add workspace", "aria-label": "Add workspace", children: _jsx(Plus, { "aria-hidden": "true" }) }), _jsxs("div", { className: "workspace-header-controls", children: [_jsxs("div", { className: "header-approval-policy-wrap", title: "How should Codex actions be approved?", children: [_jsx(ShieldCheck, { "aria-hidden": "true" }), _jsx("select", { className: "header-approval-policy", "aria-label": "How should Codex actions be approved?", value: approvalPolicy, onChange: (event) => setApprovalPolicy(event.target.value), children: APPROVAL_POLICY_OPTIONS.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) })] }), _jsxs("button", { className: "header-loop-toggle", type: "button", "aria-label": "Global Loop mode", "aria-pressed": globalLoopEnabled, "data-active": globalLoopEnabled ? "true" : undefined, disabled: globalLoopSaving, title: globalLoopEnabled ? "Loop on: automatically Grill turns with commands or file changes and start work on issues" : "Turn on global Loop mode", onClick: () => void toggleGlobalLoopMode(), children: [_jsx(Flame, { "aria-hidden": "true" }), _jsx("span", { children: globalLoopEnabled ? "Loop ON" : "Loop" })] }), _jsxs("div", { className: "header-account-popover-wrap", ref: accountPopoverRef, children: [_jsxs("button", { className: "header-account-control", type: "button", "aria-label": "Account", "aria-haspopup": "dialog", "aria-expanded": isAccountPopoverOpen, onClick: () => setIsAccountPopoverOpen((open) => !open), children: [_jsx(User, { "aria-hidden": "true" }), _jsx("span", { children: useLoadBalanceInWorkspace
                                    ...
                                    }, title: "Pending waits", "aria-label": `Pending waits${activeWaitCount > 0 ? ` (${activeWaitCount})` : ""}`, children: [_jsx(Clock3, { "aria-hidden": "true" }), activeWaitCount > 0 && (_jsx("span", { className: "pending-waits-badge", "aria-hidden": "true", children: activeWaitCount > 99 ? "99+" : activeWaitCount }))] }), _jsx("button", { className: "ghost-icon", type: "button", onClick: () => void newSessionInProjectGroup(mostRecentlyActiveProject), title: mostRecentlyActiveProject ? `New thread in ${mostRecentlyActiveProject.label}` : "New thread", "aria-label": mostRecentlyActiveProject ? `New thread in ${mostRecentlyActiveProject.label}` : "New thread", children: _jsx(Plus, { "aria-hidden": "true" }) })] }), _jsxs("div", { className: "session-list-tabs", role: "tablist", "aria-label": "Session lists", children: ["sessions", "categories"].map(tab => _jsx("button", { type: "button", role: "tab", "aria-selected": sidebarListTab === tab, onClick: () => setSidebarListTab(tab), children: tab === "sessions" ? "Sessions" : "Categories · Lab" }, tab)) }), sidebarListTab === "categories" && _jsx(SessionCategoriesPanel, { workspaceId: activeWorkspace?.id, sessionId, onOpenSession: switchToSessionById }, activeWorkspace?.id), _jsx("section", { className: "thread-tree", style: sidebarListTab === "categories" ? { display: "none" } : undefined, "aria-label": "Saved sessions", children: _jsxs("div", { className: "session-list", "data-has-selection": selectedSessionRecords.length > 0 ? "true" : undefined, children: [sessionList.length === 0 ? (_jsx("p", { className: "session-empty", children: "No saved sessions yet." })) : (groupedSessions.map((group) => {
                                    ...
                                    }))] }) }), _jsxs("section", { className: "process-monitor-panel", "aria-label": "Process monitors", children: [_jsxs("div", { className: "process-monitor-header", children: [_jsxs("span", { children: [_jsx(TerminalSquare, { "aria-hidden": "true" }), " Process monitors"] }), visibleProcessMonitors.length > 0 && _jsx("small", { children: visibleProcessMonitors.length })] }), _jsx("div", { className: "process-monitor-tabs", role: "tablist", "aria-label": "Process monitor views", children: ["running", "available"].map((tab) => _jsx("button", { type: "button", role: "tab", "aria-selected": processMonitorTab === tab, onClick: () => { setHoveredProcessMonitor(null); setProcessMonitorTab(tab); }, children: capitalize(tab) }, tab)) }), visibleProcessMonitors.length === 0 ? (_jsx("p", { className: "process-monitor-empty", children: processMonitorTab === "available" ? "No registered commands." : "No monitored processes." })) : (_jsx("div", { className: "process-monitor-list", children: visibleProcessMonitors.map((monitor) => {
                                        const isBusy = processMonitorAction?.endsWith(`:${monitor.id}`) ?? false;
                                        const latestCommandRun = monitor.status === "available"
                                            ? centralState.processMonitors.find((candidate) => candidate.sourceCommandId === monitor.id)
                                            : null;
                                        const registeredCommandRunning = latestCommandRun?.status === "running" || latestCommandRun?.status === "starting";
                                        const dockerRunArgs = Array.isArray(monitor.dockerRunArgs) ? monitor.dockerRunArgs : [];
                                        const processCommand = monitor.dockerImage
                                        ...
                                        const metricReadings = Array.isArray(monitor.metricReadings) ? monitor.metricReadings : [];
                                        const metricNameSuffix = metricReadings
                                        ...
                                        return (_jsxs("div", { className: "process-monitor-item", onMouseEnter: (event) => {
                                                ...
                                                if (!event.currentTarget.contains(event.relatedTarget)) {
                                                    ...
                                                    }, children: [_jsx("span", { className: "process-monitor-dot", "data-status": registeredCommandRunning ? "running" : monitor.status, "aria-hidden": "true" }), _jsx("span", { className: "process-monitor-label", title: rowLabel, children: rowLabel }), monitor.status !== "available" && _jsx("small", { children: monitor.status })] }), monitor.status === "available" && _jsx("button", { type: "button", className: "process-command-run", disabled: isBusy, onClick: () => void restartProcessMonitor(monitor), title: "Run command", "aria-label": "Run command", children: isBusy ? _jsx(Loader2, { className: "spin", "aria-hidden": "true" }) : _jsx(Play, { "aria-hidden": "true" }) }), hoveredProcessMonitor?.id === monitor.id && (_jsxs("section", { className: "thread-status-popover process-monitor-popover", role: "dialog", "aria-label": `${monitor.label} process details`, style: {
                            ...
                            if (event.key === "Escape")
                                setIsPendingWaitsOpen(false);
                                                    ...
                                                    return (_jsxs("tr", { children: [_jsxs("td", { className: "pending-waits-session", children: [_jsx("strong", { children: displaySessionTitle(sessionTitleById.get(subscription.sessionId) ?? shortId(subscription.sessionId)) }), _jsx("small", { children: shortId(subscription.sessionId) })] }), _jsxs("td", { className: "pending-waits-prompt", title: prompt, children: [_jsx("span", { children: prompt }), _jsxs("div", { className: "pending-waits-actions", children: [_jsx("button", { className: "ghost-icon", type: "button", onClick: () => editWaitSubscription(subscription), disabled: subscription.status === "dispatching" ||
                                                                                    ...
                                                                                    waitSubscriptionAction !== null, title: subscription.actionType === "notify" ? "Notify-only wait has no prompt" : "Edit pending prompt", "aria-label": "Edit pending prompt", children: _jsx(Pencil, { "aria-hidden": "true" }) }), _jsx("button", { className: "ghost-icon danger", type: "button", onClick: () => void removeWaitSubscription(subscription), disabled: waitSubscriptionAction !== null, title: "Remove pending wait", "aria-label": "Remove pending wait", children: waitSubscriptionAction === `remove:${subscription.id}` ? (_jsx(Loader2, { className: "spin", "aria-hidden": "true" })) : (_jsx(Trash2, { "aria-hidden": "true" })) })] })] }), _jsxs("td", { className: "pending-waits-event", title: event.subjectKey, children: [_jsx("strong", { children: event.topic }), _jsx("small", { children: event.subjectKey }), _jsx("span", { "data-status": subscription.status, children: subscription.status })] }), _jsx("td", { children: formatTimestamp(subscription.created) })] }, subscription.id));
