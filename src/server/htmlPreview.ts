...
export function createHtmlPreviewUrl(filePath: string) {
  const now = Date.now();
  ...
  grants.set(token, { root: realpathSync(dirname(filePath)), expires: now + 12 * 60 * 60 * 1000 });
  return `/api${prefix}${token}/${encodeURIComponent(basename(filePath))}`;
}
