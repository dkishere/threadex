...
const THREADEX_DEVELOPER_INSTRUCTIONS = [
  ...
  "Threadex session ownership: a parent and its forks share history only through the fork point; later work belongs to its branch. Keep a request in the receiving session unless it requires another branch's post-fork work. Do not choose a newer or higher-ranked fork from shared history alone; inspect lineage and stay current if uncertain. Context Fork/New task is explicit delegation: follow the server-provided child-task contract.",
...
function planningTurnIsReadOnly(job: RunnerJob) {
  return job.contextForkRequest === true ||
    job.forcePlan === true ||
