...
tab. Threadex uses the runner's file-change diffs saved in the database and does
not capture whole-workspace Git snapshots at turn start or during server recovery.
Each completed-turn change list uses the saved diffs to create
...
Existing legacy baselines are only consulted when historical file-change records
lack reviewable content; no new baseline or compact archive is generated by the
server. Existing baseline directories are not automatically deleted.
